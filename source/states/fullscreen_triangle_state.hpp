...
class FullscreenTriangleState : public our::State {
    ...
    void onInitialize() override {
        ...
        // We used "assets/shaders/fullscreen.vert" and
        // "assets/shaders/circle.frag" as the defaults if the paths are not
        // defined
        ...
        // Here we create a shader, attach the vertex and fragment shader to it
        // then link
        program.create();
        ...
        program.use();
        // We loop over every uniform in the configuration and send to the
        // program
        if (const auto &uniforms = config["uniforms"]; uniforms.is_object()) {
            for (auto &[name, uniform] : uniforms.items()) {
                ...
                if (type == "float") {
                    ...
                    glm::vec4 value =
                        uniform.value("value", glm::vec4(0, 0, 0, 0));
                    program.set(name, value);
                }
            }
        }
